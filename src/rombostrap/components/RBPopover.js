...
export class RBPopoverTarget extends PureComponent {
  render() {
    const {
      abuttedLeft,
      pinnedRight,
...
export class RBPopoverContainer extends PureComponent {
  render() {
    const {
      classes,
      abuttedLeft,
...
export const RBPopoverContent = ({ classes, children }) => (
  <div className={classnames('vd-popover-content', classes)}>{children}</div>
);

export const RBPopoverList = ({ children }) => (
  <ol className="vd-popover-list">{children}</ol>
);

export const RBPopoverListHeader = ({ children }) => (
  <li className="vd-popover-list-header">{children}</li>
);

export const RBPopoverListGroup = ({ children }) => (
  <li className="vd-popover-list-group">{children}</li>
);

export const RBPopoverListItem = ({ active, children }) => (
  <a
...
export const RBPopoverActions = ({ children }) => (
...
class RBPopover extends PureComponent {
  state = {};
  componentDidMount() {
  ...
  componentWillReceiveProps(nextProps) {
    const nodeClassName = `vd-${nextProps.id}-container`;
    let MOUNT;
    if (nextProps.open) {
    ...
    if (this.props.open) {
      ...
      setTimeout(() => {
        ...
        if (targetBottom && containerTop) {
          ...
          translateX = rectTarget.left;
          translateY = rectTarget.bottom;
          if (
          ...
          if (
            ...
            translateY =
              window.innerHeight - node.clientHeight - POPOVER_PADDING;
          }
        } else if (
          targetMiddle &&
          targetLeft &&
